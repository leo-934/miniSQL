...
catalog CatalogManager::getCataByAttrName(std::string tableName, std::string attrName)
{
	...
	if (tmp == catas.end())
		throw std::runtime_error("no this table");
	auto res = tmp->second.find(attrName);
	if (res == tmp->second.end())
	{
		std::cout << "123";
		throw std::runtime_error("no this attr");
	}
	return res->second;
}
	...
	for (auto j : originalAttrNames[tableName])
	{
		for (auto i : catas[tableName])
		{
			if (i.first == j)
			{
				...
				if (i.second == catalog::CHAR)
					res.push_back(static_cast<int>(attrLenForChars[tableName][i.first]));
			}
		}
	}
	/*for (auto i : catas[tableName]) {
...
{
	std::vector<std::string> res;
	if (catas.find(tableName) == catas.end())
		throw std::runtime_error("no this table");
	for (auto i : catas[tableName])
		res.push_back(i.first);
	return res;
...
void CatalogManager::dropTable(std::string tableName)
{
	...
	if (primaryKeys.find(tableName) != primaryKeys.end())
		primaryKeys.erase(primaryKeys.find(tableName));
	if (uniqueKeys.find(tableName) != uniqueKeys.end())
